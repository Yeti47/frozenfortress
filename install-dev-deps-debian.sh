...
# FrozenFortress Development Dependencies Installer for Debian/Ubuntu
...
check_package_manager() {
    if ! command -v apt &> /dev/null; then
        log_error "apt package manager not found. This script is designed for Debian/Ubuntu systems."
        exit 1
    fi
}
...
install_redis() {
    ...
    log_success "Redis server installed and started successfully"
}
...
install_tesseract_languages() {
    ...
    # Available common languages (Debian/Ubuntu package names)
    ...
    for lang_code in "${selected_languages[@]}"; do
        local package_name="tesseract-ocr-$lang_code"
...
verify_installations() {
    ...
    # Check Redis
    if command -v redis-server &> /dev/null; then
        log_success "Redis server is installed"
